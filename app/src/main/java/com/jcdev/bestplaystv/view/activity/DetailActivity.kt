...
class DetailActivity : PlaysActivity(), CastPlayer.SessionAvailabilityListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null && (bundle.containsKey("game") || bundle.containsKey("user"))) {
            ...
            //InitModelView
            val viewModel =
                ViewModelProviders.of(this, viewModelFactory { DetailViewModel(id, type) })
                    .get(DetailViewModel::class.java)

            videoAdapter = VideoAdapter(ArrayList(0)) { video: Video ->
    ...
    fun connectChromecast(video: Video) {
        ...
        val videoUrl = "https://d1playscdntv-a.akamaihd.net/" +
                if (video.thumbnail.contains("media")) "media/" else "video/"  +
                video.videoId +
                        if (video.thumbnail.contains("transcoded")) "/transcoded/" else "/processed/" +
                resolutions.first() +
                ".mp4"
